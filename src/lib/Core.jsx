...
const Core = ({ questions, appLocale, showDefaultResult, onComplete, customResultPage, showInstantFeedback, continueTillCorrect }) => {
  ...
  const renderAnswers = (question, buttons) => {
    const { answers, correctAnswer, questionType } = question;
    let { answerSelectionType } = question;
    const onClickAnswer = (textInput) => {
      let actualIndex;
      if(answers.includes(textInput)){
        actualIndex = correctAnswer;
      }else {
        actualIndex = -20;
      }
      checkAnswer(actualIndex, textInput, correctAnswer, answerSelectionType, {
        userInput,
        userAttempt,
        currentQuestionIndex,
        continueTillCorrect,
